...
if (file_exists(GUS_cachefile()) AND ($today != $month . $year)) {
	...
	for ($i = 0; $i < $nrows; $i++) {
		...
		$T->set_var(array(
			'colclass' => 'col_right',
			'data'     => '<a href="' . $_CONF['site_url'] . "/gus/referer.php?year=$year&amp;month=$month&amp;referrer={$referrer}" . '">' . $A['count'] . '</a>',
		));
		$T->parse('CBlock', 'COLUMN', FALSE);
		$T->set_var(array(
			'colclass' => 'col_left',
			'data'     => '<a href="http://' . $referrer . '" target="_blank">' . htmlentities($referrer) . '</a>',
		));
		$T->parse('CBlock', 'COLUMN', TRUE);
		$T->parse('BBlock', 'ROW', TRUE);
	}
